...
bool FightView::load()
{
    ...
    this->action.setString("C'est � votre tour d'attaquer !");
...
void FightView::render(sf::RenderWindow& window)
{
    ...
    if (this->attacking)
    {
        ...
        this->attack_name.setString("Sortil�ges disponibles");
                    ...
                    else
                    {
                        ...
                        if (0 <= pos_atk_sel && pos_atk_sel < this->equip->getSize() && this->equip->getCrea(pos_atk_sel)->getLife() > 0)
                        {
                                 ...
                                 s == UniqueTargetAdvParalize || s == UniqueTargetUsHeal)
                            {
                                ...
                                this->action.setString("Sur quelle cr�ature souhaitez-vous utiliser le sort ?");
                            }
                            else
                            {
                                this->__selected = 42;  // special code to tell the engine to chose random creatures
                                this->action.setString("Attaque multiple d�clench�e");
...
void FightView::update(sf::RenderWindow& window, sf::Time elapsed)
{
    ...
    if (this->wait_give_xp == 1)
    {
        ...
        if (this->whoisdead == DEADOTH)
            this->action.setString("Les cr�atures ennemies ont perdues !");
    ...
    if (this->wait_give_xp % 360 == 0 && this->wait_give_xp != 0 && this->giving_xp_to != -1)
    {
        ...
        if (i < m)
        {
            ...
            if (this->giving_xp_to == 1)
            {
                ...
                this->action.setString(this->equip->getCrea(i)->getName() + " a gagn� " + to_string<int>(t) + " niveau" + ((t == 1) ? std::string("") : std::string("x")));
            }
            else
            {
...
void FightView::check_statuses()
{
    ...
    for (int i=0; i < this->adv.size(); ++i)
    {
        ...
        case DEAD_PSN:
            s += std::string("L'ennemi ") + this->adv[i]->getName() + " est mort empoisonn�\n";
            break;

        case LOST_BUR:
            s += std::string("L'ennemi ") + this->adv[i]->getName() + " n'est plus br�l� !\n";
            break;

        case LOST_PAR:
            s += std::string("L'ennemi ") + this->adv[i]->getName() + " n'est plus paralys� !\n";
            break;

        case LOST_PSN:
            s += std::string("L'ennemi ") + this->adv[i]->getName() + " n'est plus empoisonn� !\n";
            break;
    ...
    for (int i=0; i < this->equip->getSize(); ++i)
    {
        ...
        case DEAD_PSN:
            s += this->equip->getCrea(i)->getName() + " est mort empoisonn�\n";
            break;

        case LOST_BUR:
            s += this->equip->getCrea(i)->getName() + " n'est plus br�l� !\n";
            break;

        case LOST_PAR:
            s += this->equip->getCrea(i)->getName() + " n'est plus paralys� !\n";
            break;

        case LOST_PSN:
            s += this->equip->getCrea(i)->getName() + " n'est plus empoisonn� !\n";
            break;
...
void FightView::e_attack(int selected)
{
    ...
    else
    {
        ...
        else
        {
            ...
            this->action.setString(std::string("L'ennemi ") + my->getName() + " aide son alli� " + enemy->getName());
            my->attack(enemy);
